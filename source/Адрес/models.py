...
class Адрес(models.Model):
    регионы = (
        ...
        ('09', '09 Республика Карачаево-Черкесия'),
    ...
    индекс = models.CharField(max_length=6, blank=True, null=True)
    регион = models.CharField(max_length=6, choices=регионы)
    город = models.CharField(max_length=50, blank=True, null=True)
    улица = models.CharField(max_length=50, blank=True, null=True, verbose_name='улица, дом, офис')
    ...
    def __str__(self):
        return str(self.индекс + ", " if self.индекс else '') + str(self.город + ", " if self.город else '') + str(self.улица if self.улица else '')